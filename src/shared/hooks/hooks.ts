import { HookAction, HookFilter, THooks } from '~/shared/hooks/index';
...
export const DASHBOARD_HOOK                = 'renderer.dashboard',
             DASHBOARD_HOOK_WIDGETS        = `${DASHBOARD_HOOK}.widgets`,
             DASHBOARD_HOOK_SAGAS          = `${DASHBOARD_HOOK}.sagas`,
             DASHBOARD_HOOK_STORE_REDUCERS = `${DASHBOARD_HOOK}.reducers`,
             DASHBOARD_HOOK_STORE_INIT     = `${DASHBOARD_HOOK}.initialState`
;

export const MAINPAGE_HOOK                = 'renderer.MAINPAGE',
             MAINPAGE_HOOK_WIDGETS        = `${MAINPAGE_HOOK}.widgets`,
             MAINPAGE_HOOK_SAGAS          = `${MAINPAGE_HOOK}.sagas`,
             MAINPAGE_HOOK_STORE_REDUCERS = `${MAINPAGE_HOOK}.reducers`,
             MAINPAGE_IPC_QUERY            = `${MAINPAGE_HOOK}.ipcQuery`,
             MAINPAGE_HOOK_STORE_INIT     = `${MAINPAGE_HOOK}.initialState`
;
 ...
 */
// @ts-ignore
var Hooks: THooks = Hooks || {}; // Extend Hooks if exists or create new Hooks object.
 ...
 */
Hooks.add_action = function (tag, callback, priority = 10) {
 ...
 */
Hooks.add_filter = function (tag: string | string[], callback, priority = 10) {
    let ltag = typeof tag === 'string' ? [tag] : tag;

    ltag.forEach(t => {
        // If the tag doesn't exist, create it.
        Hooks.filters[t] = Hooks.filters[t] || [];
        Hooks.filters[t].push({priority: priority, callback: callback});
